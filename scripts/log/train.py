...
dims, max_epochs, batch_size = cfg['dims'], cfg['max_epochs'], cfg['batch_size']
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            ...
            X_pl = tf.placeholder(tf.float32, shape=[batch_size,32,32,32], name='X')  # X = T.TensorType('float32', [False]*5)('X')
